...
function Install-LocalLauncher {
    ...
    # Retorna $true se REALMENTE copiou (ou seja, esta execução veio de um
    # caminho remoto/GPO) - usado pra decidir se aplica o delay pós-conexão.
    if ($PSCommandPath -and ($PSCommandPath -eq $LauncherLocalPath)) {
        return $false
    }

    Write-Log "Executando a partir de um caminho remoto ($PSCommandPath). Instalando/atualizando cópia local..."
    ...
    # Limpeza de tarefas de modelos anteriores (migração)
    foreach ($nomeAntigo in @(
        "FirstDecision-LauncherLocal-Logon", "FirstDecision-LauncherLocal-Periodico", "FirstDecision-LauncherLocal",
        "TASK POPUP - FD"   # sistema antigo de popup de bem-estar (GPO "POP_UP", já substituído por Notificacao-BemEstar.ps1)
    )) {
        if (Get-ScheduledTask -TaskName $nomeAntigo -ErrorAction SilentlyContinue) {
            Unregister-ScheduledTask -TaskName $nomeAntigo -Confirm:$false -ErrorAction SilentlyContinue
            Write-Log "Tarefa local obsoleta '$nomeAntigo' removida."
        }
    }
...
$vieioDeRemoto = Install-LocalLauncher -VbsPath $vbsPath
